...
int main(int argc,char *argv[]){
    ...
    len = sbuf.st_size;
    printf("the size of srcFile is %d\n",len);
    if (len < n){
        n = len;
    }
    //根据文件大小拓展目标文件
    ...
    int mod = len % bs;  //求出均分后余下的字节数,让最后一个子进程处

    //在子进程中，fork函数返回0，在父进程中，fork返回子进程的进程ID,因此, 我们可以通过fork返回的值来判断当前进程是子进程还是父进程
    //创建N个子进程
    for (i = 0; i<n; i++){
        if( (pid = fork() ) == 0){
            break;
